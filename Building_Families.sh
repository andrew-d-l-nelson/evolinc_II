...
while getopts ":l:q:s:e:k:hg:" opt; do
  case $opt in
    l)
      lincRNAfasta=$OPTARG
    ;;
...
makeblastdb -in $subject_gff -dbtype nucl -out BLAST_DB/$subject_gff.fasta.blast.out
...
makeblastdb -in $known_lincRNAs -dbtype nucl -out BLAST_DB/$known_lincRNAs.blast.out

blastn -query BLAST_Return/$subject_species.$query_species.orthologs.renamed.fasta -db BLAST_DB/$known_lincRNAs.blast.out -num_threads 4 -penalty -2 -reward 1 -gapopen 5 -gapextend 2 -dust no -word_size 8 -evalue 1e-20 -outfmt "6 qseqid sseqid pident length qlen qstart qend sstart send evalue bitscore" -out BLAST_Return/$subject_species.$query_species.orthologs.renamed.lincRNAs_tested.out
#An error prompt will show up if no subject_gff is provided. This is okay!

python filter_lincRNA_sequences_annotation.py BLAST_Return/$subject_species.$query_species.orthologs.renamed.lincRNAs_tested.out BLAST_Return/$subject_species.lincRNA_annotation.list.txt

python assign_annotation_lincRNA.py BLAST_Return/$subject_species.$query_species.orthologs.renamed.fasta BLAST_Return/$subject_species.lincRNA_annotation.list.txt BLAST_Return/$subject_species.$query_species.orthologs.lincRNA_tested.renamed.fasta

perl Sort_FASTA_Alp.pl -r BLAST_Return/$subject_species.$query_species.orthologs.lincRNA_tested.renamed.fasta >BLAST_Return/$subject_species.$query_species.orthologs_alpha.fasta
